...
public class TestTrackerWithRollBack {
	...
	@Test
	public void testForGCMonitoring() throws InterruptedException {
		//время для запуска программы для профилирования
		Thread.sleep(10000);
		...
		try (Store sqlTracker = new SQLTracker((ConnectionRollback.create(this.init())), "items")) {
			...
			for (int i = 0; i < 20_000; i++) {
				//создание объектов
				Item item = new Item("newItem" + i);
				//создание мусора
				Item itemForDelete = new Item("для удаления");
				...
				if (i % 200 == 0) {
					...
					for (; l < k + 20; l++) {
						...
						Thread.sleep(5);
						boolean delete = sqlTracker.delete(String.valueOf(l));
						System.out.println("удалена запись из БД " + l);
					}
				}
				if (i % 5000 == 0) {
					Thread.sleep(5);
